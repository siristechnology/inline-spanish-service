...
describe('link-crawler', () => {
	it('should fetch links from medium.com successfully', async () => {
		const sourceConfigs = JSON.parse(fs.readFileSync(path.resolve(__dirname, '../config/source-configs.json'), 'utf8'))
	...
	it('should fetch links from cnn.com/special successfully', async () => {
		...
		cnnSourceConfig[0].links = cnnSourceConfig[0].links.filter(link => link.url === 'https://www.cnn.com/specials/last-50-stories')
